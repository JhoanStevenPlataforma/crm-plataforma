...
export const generateSales = (_: Db): Sale[] => {
  const randomSales: Sale[] = Array.from(Array(5).keys()).map((id) => {
    ...
    return {
      ...
      email,
      password: "demo",
      // The first generated user is a sales manager so the demo shows the
      // manager/rep split; the rest are plain reps.
      role: id === 0 ? "manager" : "rep",
      disabled: false,
    };
  });
  return [
