...
### Two Non-Obvious Causes of Phantom Diffs

PostgreSQL stores a function body as literal text, so a hand-written migration
and the schema file must match **byte for byte**, comments and all.

1. **Line endings.** This repository uses CRLF. A migration file written with LF
   endings stores a different body than the CRLF schema file, and `db diff` will
   re-emit the function on every run even though the code is identical. Check
   with `file supabase/migrations/<name>.sql` and normalise to CRLF.
2. **Column order.** PostgreSQL cannot insert a column in the middle of a table.
   A migration that replaces a column (`add` the new one, backfill, `drop` the
   old one) leaves the new column **at the end**, while the schema file usually
   declares it where the old one was. Declare it last in `01_tables.sql` to
   match, with a comment explaining why. `public.sales.role` is the example in
   this codebase.

A useful check, since the repository already carries some unrelated
`grant ... to anon` drift: filter it out and confirm nothing else remains.
